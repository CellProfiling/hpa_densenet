...
def predict_hpa_images(image_ids, work_dir="./data", threshold=0.5, model_path=None):
    ...
    if not os.path.exists(model_path):
        ...
        urllib.request.urlretrieve(
            "https://github.com/CellProfiling/densenet/releases/download/v0.1.0/external_crop512_focal_slov_hardlog_class_densenet121_dropout_i768_aug2_5folds_fold0_final_probability.onnx",
            model_path,
        )
    ...
    for image_id in image_ids:
        ...
        input_image = input_image.reshape(
            1, 4, input_image.shape[1], input_image.shape[2]
        )
        classes, features = ort_session.run(None, {"image": input_image / 255.0})
        pred = [
            (LABELS[i], prob)
            for i, prob in enumerate(classes[0].tolist())
            if prob > threshold
        ]
