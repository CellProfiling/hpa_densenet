...
class Config(object):
    ...
    model_path = "models/bestfitting_default_model.pth"

    suffix = "png"

    image_dir = "/mnt/data/resized_hpa_ifimages/"
    result_name = "hpa22"
    out_dir = "/mnt/data/v22_hpa_features/"
...
def export_onnx(model, file_path="densenet_model.onnx"):
    ...
    exported_results = ort_session.run(
        None, {"image": example.numpy().astype(np.float32)}
    )
    with torch.no_grad():
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--gpus", type=str, default="", help="GPU device numbers, e.g. 0,1"
    )
    parser.add_argument(
        "--image_dir",
        type=str,
        default="../resized_images/images_1536",
        help="input image folder",
    )
    parser.add_argument(
        "--out_dir", type=str, default="../results/", help="output image folder"
    )
